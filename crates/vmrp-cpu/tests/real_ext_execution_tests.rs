...
fn pop_like_block_transfer_restores_registers_and_pc() {
    ...
    cpu.memory_mut().write32(GuestAddr::new(0x8FFF8), 0x4444_4444).unwrap();
    cpu.memory_mut().write32(GuestAddr::new(0x8FFFC), 0x81234).unwrap();
...
fn literal_load_reads_word_from_pc_relative_address() {
    ...
    cpu.memory_mut().write32(GuestAddr::new(0x80114), 0x1122_3344).unwrap();
...
fn offset_load_reads_word_from_base_minus_offset() {
    ...
    cpu.memory_mut().write32(GuestAddr::new(0x88008), 0xAABB_CCDD).unwrap();
...
fn conditional_post_index_store_updates_memory_and_base_when_taken() {
    ...
    assert_eq!(cpu.memory().read32(GuestAddr::new(0x88010)).unwrap(), 0xAABB_CCDD);
...
fn ldmia_without_writeback_loads_registers_and_advances_pc() {
    ...
    cpu.memory_mut().write32(GuestAddr::new(0x88000), 0x11).unwrap();
    cpu.memory_mut().write32(GuestAddr::new(0x88004), 0x22).unwrap();
    cpu.memory_mut().write32(GuestAddr::new(0x88008), 0x33).unwrap();
...
fn stmib_sp_without_writeback_stores_r2_r3() {
    ...
    assert_eq!(cpu.memory().read32(GuestAddr::new(0x90004)).unwrap(), 0x2222_2222);
    assert_eq!(cpu.memory().read32(GuestAddr::new(0x90008)).unwrap(), 0x3333_3333);
    assert_eq!(cpu.regs().sp(), 0x90000);
    assert_eq!(cpu.regs().pc(), 0x80004);
}
