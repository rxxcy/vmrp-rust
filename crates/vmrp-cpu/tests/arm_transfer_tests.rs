...
fn arm_ldr_word_from_unaligned_address_rotates_loaded_value() {
    ...
    cpu.memory_mut().write8(GuestAddr::new(0x80004), 0x04).unwrap();
    cpu.memory_mut().write8(GuestAddr::new(0x80005), 0x05).unwrap();
    cpu.memory_mut().write8(GuestAddr::new(0x80006), 0x06).unwrap();
    cpu.memory_mut().write8(GuestAddr::new(0x80007), 0x07).unwrap();
...
fn arm_str_word_to_unaligned_address_uses_aligned_word_address() {
    ...
    assert_eq!(cpu.memory().read32(GuestAddr::new(0x80004)).unwrap(), 0x1122_3344);
    assert_eq!(cpu.regs().pc(), 0x80004);
}
...
fn arm_ldrh_immediate_loads_halfword() {
    ...
    cpu.memory_mut().write16(GuestAddr::new(0x80018), 0x1234).unwrap();
...
fn arm_ldrsh_immediate_sign_extends_halfword() {
    ...
    cpu.memory_mut().write16(GuestAddr::new(0x80018), 0xFF80).unwrap();
...
fn arm_strh_immediate_stores_halfword() {
    ...
    assert_eq!(cpu.memory().read16(GuestAddr::new(0x80018)).unwrap(), 0x1234);
    assert_eq!(cpu.regs().pc(), 0x80004);
}
